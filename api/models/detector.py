...
class AnomalyDetector:
    ...
    def __init__(self):
        self.if_model = self.lof_model = self.scaler = None
        self.feature_cols = self.thresholds = self.evaluation = None
        self._loaded = False
    ...
    def _normalise(self, raw_if: float, raw_lof: float):
        if_score  = float(np.clip(0.5 - raw_if,    0, 1))
        lof_score = float(np.clip((-raw_lof) / 2.0, 0, 1))
        ensemble  = 0.6 * if_score + 0.4 * lof_score
    ...
    def predict(self, txn: TransactionRequest) -> AnomalyResponse:
        ...
        X       = self.scaler.transform(self._build_features(txn))
        raw_if  = float(self.if_model.score_samples(X)[0])
        raw_lof = float(self.lof_model.score_samples(X)[0])
        inf_ms  = (time.perf_counter() - t0) * 1000
        if_s, lof_s, ens = self._normalise(raw_if, raw_lof)
